...
public class PetCount2 {
	public static void main(String[] args)
		...
		//Class[] petTypes = {Class.forName("Dog"),Class.forName("Pug"),Class.forName("Cat"),Class.forName("Rodent"),Class.forName("Gerbil"),Class.forName("Hamster"),};
			
		Class[] petTypes = {Pet.class,Dog.class,Pug.class,Cat.class,Rodent.class,Gerbil.class,Hamster.class,};	// Classi letterali

		try {
			for(int i = 0; i < 15; i++) {
				int rnd = 1 + (int)(Math.random() * (petTypes.length - 1));			// Offset by one to eliminate Pet.class:
		...
		Iterator keys = h.keySet().iterator();	//hashmap 
		while(keys.hasNext()) {
