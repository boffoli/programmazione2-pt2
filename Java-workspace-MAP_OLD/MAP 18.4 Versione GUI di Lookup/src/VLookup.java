...
public class VLookup extends JApplet {
	...
	public void init() {
			searchFor.getDocument().addDocumentListener(new SearchL());
				...
				try {					
					Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");	// Carica il driver (si registra)
	...
	public void textValueChanged() {
		...
		try {// Name completion:
			r = s.executeQuery("SELECT LAST FROM people.csv people " + "WHERE (LAST Like '" + searchFor.getText() + "%') ORDER BY LAST");
			if(r.next())
				completion.setText(r.getString("last"));
				r = s.executeQuery("SELECT FIRST, LAST, EMAIL " + "FROM people.csv people " + "WHERE (LAST='" + completion.getText() + "') AND (EMAIL Is Not Null) " + "ORDER BY FIRST");
		} 
		catch(Exception e) {
