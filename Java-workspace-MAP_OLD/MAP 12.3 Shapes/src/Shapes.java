...
public class Shapes {
	public static Shape randShape() {
		switch((int)(Math.random() * 3)) {	//fra 0 e 2
			default:
			case 0: 
				return new Circle();
			case 1: 
	...
	public static void main(String[] args) {
		Shape[] s = new Shape[9];	//array di oggetti di una classe
		for(int i = 0; i < s.length; i++)
			s[i] = randShape();

		for(int i = 0; i < s.length; i++)
			s[i].draw();			// chiamate polimorfiche: in base all'oggetto lui capisce qule disegnare
		}
}
